...
import './style.css';
function App(){
 const [user,setUser]=useState<Row|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(''),[screen,setScreen]=useState('dashboard'),[email,setEmail]=useState(''),[senha,setSenha]=useState('');
 ...
 return <div className="shell"><aside><div className="brand">duali<span>GESTÃO DE PESSOAS</span></div><nav>{[['dashboard','Vis?o geral'],['relatorios','Relat?rios'],['auditoria','Auditoria']].map(([key,title])=><button key={key} className={screen===key?'active':''} onClick={()=>setScreen(key!)}>{title}</button>)}<button className={screen==='importacoes'?'active':''} onClick={()=>setScreen('importacoes')}>Importa??es</button><button className={screen==='saldos'?'active':''} onClick={()=>setScreen('saldos')}>Saldo e hist?rico</button>{screens.map(s=><button className={s.path===screen?'active':''} key={s.path} onClick={()=>setScreen(s.path)}>{s.title}</button>)}</nav><div className="account"><strong>{String(user.nome)}</strong><span>Administrador</span><button className="secondary" onClick={()=>{void api('auth/logout','POST',{}).then(()=>{setUser(null);setCsrf('');}).catch(e=>setError((e as Error).message));}}>Sair</button></div></aside><main><header className="topbar"><span>Operação / {selected.title}</span><span>Duali · Gestão de Pessoas</span></header><Notice text={error} error/>{screen==='dashboard'?<Dashboard/>:screen==='relatorios'?<Reporting/>:screen==='auditoria'?<Audit/>:screen==='importacoes'?<Imports/>:screen==='saldos'?<Ledger/>:<Records key={screen} screen={selected}/>}</main></div>;
}
