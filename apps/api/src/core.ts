...
export interface Delegate {
 findMany(args?:Row):Promise<Row[]>; findUnique(args:Row):Promise<Row|null>;
 findFirst(args?:Row):Promise<Row|null>; count(args?:Row):Promise<number>;
 create(args:Row):Promise<Row>; update(args:Row):Promise<Row>;
}
export function model(db:Tx,name:string):Delegate {return (db as unknown as Record<string,Delegate>)[name]!;}
export function json(value:unknown):Prisma.InputJsonValue {return JSON.parse(JSON.stringify(value)) as Prisma.InputJsonValue;}
export async function audit(tx:Tx,userId:string|null,acao:string,entidade:string,entidadeId:string|null,before?:unknown,after?:unknown){
