...
export async function acquire(
  ...
  let count = 0;
  for (const event of entitlements(
    vinculo.dataAdmissao,
    vinculo.tipo,
    until,
    vinculo.dataDesligamento,
  )) {
    const existing = await tx.descansoDireito.findUnique({
      where: {
        vinculoId_dataAquisicao: {
          vinculoId,
          dataAquisicao: event.dataAquisicao,
        },
      },
    });
    if (existing) continue;
    const result = await tx.descansoDireito.createMany({
      data: { vinculoId, ...event, origem: "AUTOMATICA" },
      skipDuplicates: true,
    });
    if (result.count) {
      await audit(
...
export async function synchronize(db: PrismaClient, userId: string | null) {
  ...
  for (;;) {
    ...
    for (const v of items)
      created += await transaction(db, (tx) => acquire(tx, v.id, userId));
...
export async function balance(tx: Tx, vinculoId: string) {
  const [rights, adjustments, consumptions, periods, v] = await Promise.all([
    tx.descansoDireito.findMany({
      where: { vinculoId },
      orderBy: { dataAquisicao: "asc" },
    }),
    tx.descansoAjuste.findMany({ where: { vinculoId } }),
    tx.descansoConsumo.findMany({
      where: { periodo: { vinculoId, status: { not: "CANCELADO" } } },
    }),
    tx.descansoPeriodo.findMany({ where: { vinculoId } }),
    tx.vinculo.findUniqueOrThrow({
      where: { id: vinculoId },
  ...
  const pending = rights.map((r) => ({
    ...
    pendente: Math.max(
      0,
      Number(r.quantidadeDias) -
        consumptions
          .filter((c) => c.direitoId === r.id)
          .reduce((sum, c) => sum + Number(c.quantidadeDias), 0),
    ),
  }));
  const alerts: string[] = [];
  ...
  if (
    v.dataDesligamento &&
    rights.some((r) => r.dataAquisicao > v.dataDesligamento!)
  )
    alerts.push(
      "Direitos posteriores ao desligamento: revisar com ajuste auditado.",
    );
  for (const r of pending.filter((r) => r.pendente > 0)) {
    if (v.tipo === "CLT" && !r.prazoConcessivo)
      alerts.push("Prazo concessivo não informado.");
    if (r.prazoConcessivo) {
      const days = (r.prazoConcessivo.getTime() - now.getTime()) / 86400000;
      if (days < 0) alerts.push("Férias vencidas com direito pendente.");
      else if (days <= v.unidade.diasAlerta)
        alerts.push("Prazo de férias próximo.");
    }
    if (rights.some((next) => next.dataAquisicao > r.dataAquisicao))
      alerts.push("Nova aquisição com direito anterior pendente.");
  }
  const next = entitlements(
    v.dataAdmissao,
    v.tipo,
    new Date(now.getTime() + v.unidade.diasAlerta * 86400000),
    v.dataDesligamento,
  ...
  return {
    ...
    programados: periods
      ...
      .reduce((sum, p) => sum + Number(p.quantidadeDias), 0),
    direitos: pending,
    periodos: periods,
