...
export const resources: Resource[] = [
  ...
  { path: "equipes", model: "equipe", schema: equipeSchema, search: "nome" },
  {
    ...
    dates: ["dataAdmissao", "dataDesligamento"],
    ...
    before: async (tx, data, previous) => {
      if (
        ...
        (previous.tipo !== data.tipo ||
          String(previous.pessoaId) !== data.pessoaId ||
          String((previous.dataAdmissao as Date).toISOString().slice(0, 10)) !==
            (data.dataAdmissao as Date).toISOString().slice(0, 10))
      )
    ...
    after: async (tx, current, previous) => {
      if (current.equipeId !== previous?.equipeId) {
        ...
        await tx.vinculoEquipeHistorico.updateMany({
          where: { vinculoId: String(current.id), fimEm: null },
          data: { fimEm: now },
        });
        if (current.equipeId)
          await tx.vinculoEquipeHistorico.create({
            data: {
              vinculoId: String(current.id),
              equipeId: String(current.equipeId),
              inicioEm: now,
            },
          });
      }
  ...
  },
  {
    ...
    before: async (tx, data, previous, userId) => {
      if (previous?.id === userId && data.ativo === false)
        throw new DomainError(
          422,
      ...
      delete data.senha;
      if (previous && (data.senhaHash || data.ativo === false))
        await tx.sessao.deleteMany({
          where: { usuarioId: String(previous.id) },
        });
    },
  },
];
export function queryFor(
  resource: Resource,
  query: ReturnType<typeof listSchema.parse>,
): Row {
  const where: Row = {};
  ...
  if (query.q && !resource.search) {
    if (resource.model === "vinculo")
      where.pessoa = {
        nomeCompleto: { contains: query.q, mode: "insensitive" },
      };
    else if (resource.filters?.includes("vinculoId"))
      where.vinculo = {
        pessoa: { nomeCompleto: { contains: query.q, mode: "insensitive" } },
      };
  }
  if (query.q && resource.search)
    where[resource.search] = { contains: query.q, mode: "insensitive" };
...
export async function saveResource(
  ...
  const data = dateData(
    resource.schema.parse(input) as Row,
    resource.dates ?? [],
  );
  ...
  const current = recordId
    ? await delegate.update({ ...args, where: { id: recordId } })
    : await delegate.create(args);
  await resource.after?.(tx, current, previous, userId);
  if (generateRights && resource.model === "vinculo")
    await acquire(tx, String(current.id), userId);
  await audit(
    tx,
    userId,
    recordId ? "ALTERAR" : "CRIAR",
    resource.model,
    String(current.id),
    previous ?? undefined,
    current,
...
export function registerResource(
  ...
  app.get("/api/" + resource.path, async (req) => {
    const query = listSchema.parse(req.query);
    const where = queryFor(resource, query);
    const delegate = model(db, resource.model);
    ...
    return {
      items: resource.present ? items.map(resource.present) : items,
      total,
      page: query.page,
      pageSize: query.pageSize,
    };
  });
  app.get("/api/" + resource.path + "/:id", async (req) => {
    const { id } = paramsId.parse(req.params);
    const item = await model(db, resource.model).findUnique({
      where: { id },
      ...(resource.include ? { include: resource.include } : {}),
      ...(resource.select ? { select: resource.select } : {}),
    });
    if (!item) throw new DomainError(404, "Registro não encontrado.");
    return resource.present ? resource.present(item) : item;
  });
  app.post(
    "/api/" + resource.path,
    ...
    async (req, reply) => {
      const result = await transaction(db, (tx) =>
        saveResource(tx, resource, req.body, req.userId!),
      );
      reply.code(201);
      return result;
    },
  ...
  app.put(
    ...
    async (req) => {
      ...
      return transaction(db, (tx) =>
        saveResource(tx, resource, req.body, req.userId!, id),
      );
    },
  );
}
export function registerPeople(app: FastifyInstance, db: PrismaClient) {
  ...
  app.get("/api/vinculos/:id/equipes-historico", async (req) => {
    const { id } = paramsId.parse(req.params);
    return db.vinculoEquipeHistorico.findMany({
      where: { vinculoId: id },
      include: { equipe: true },
