...
const querySchema = listSchema.extend({
  severidade: z.enum(["CRITICA", "ATENCAO", "REVISAO", "INFORMATIVA"]).optional(),
  codigo: z.string().max(80).optional(),
  modulo: z.enum(["IMPORTACAO", "VINCULO", "ESTAGIO", "DESCANSO", "BENEFICIO"]).optional(),
...
function legacyCode(status: string, messages: unknown): string {
  ...
  if (status === "DUPLICIDADE" || text.includes("duplic")) return "DUPLICIDADE_POSSIVEL";
  if (text.includes("cpf")) return "CPF_INVALIDO";
  if (text.includes("ambígua") || text.includes("ambigua")) return "DATA_AMBIGUA";
...
async function allPendings(db: PrismaClient): Promise<PendingItem[]> {
  const [imports, internship, leave, benefits, activeLinks] = await Promise.all([
    db.importacaoItem.findMany({
      where: { status: { in: ["REVISAO", "DUPLICIDADE", "AGUARDANDO_DEPENDENCIA"] } },
  ...
  const result: PendingItem[] = imports.map((item) => {
    ...
    const stored = Array.isArray(item.inconsistencias) ? item.inconsistencias[0] as Record<string, unknown> | undefined : undefined;
    const codigo = String(stored?.codigo ?? legacyCode(item.status, item.mensagens));
    return {
      id: `importacao:${item.id}`,
      codigo,
      severidade: item.status === "AGUARDANDO_DEPENDENCIA" ? "ATENCAO" : "REVISAO",
      modulo: "IMPORTACAO",
      ...(typeof normalized.nomeCompleto === "string" ? { pessoa: normalized.nomeCompleto } : {}),
      ...(typeof normalized.pessoaId === "string" ? { pessoaId: normalized.pessoaId } : {}),
      status: item.status,
      origem: `${item.importacao.nomeArquivo} · ${item.aba}:${item.numeroLinha}`,
      descricao: Array.isArray(item.mensagens) ? item.mensagens.map(String).join("; ") : codigo,
      href: `/app/importacoes?id=${item.importacaoId}&item=${item.id}`,
    };
  });
  for (const alert of [...internship, ...leave, ...benefits]) {
    const module = alert.tipo === "BENEFICIO" ? "BENEFICIO" : alert.tipo === "DESCANSO" ? "DESCANSO" : "ESTAGIO";
    const code = alert.tipo === "BENEFICIO" ? "AJUSTE_BENEFICIO_PENDENTE" : alert.tipo === "DOCUMENTO" ? "DOCUMENTO_VENCIDO" : alert.tipo === "DESCANSO" && alert.mensagem.toLowerCase().includes("negativo") ? "SALDO_NEGATIVO" : "REFERENCIA_NAO_ENCONTRADA";
    result.push({
      ...
      ...(typeof alert.unidadeId === "string" ? { unidadeId: alert.unidadeId } : {}),
      ...("equipeId" in alert && typeof alert.equipeId === "string" ? { equipeId: alert.equipeId } : {}),
      status: "ABERTA",
      origem: module,
      descricao: alert.mensagem,
      ...(alert.prazo instanceof Date ? { prazo: alert.prazo } : {}),
      href: alert.vinculoId ? `/app/pessoas?vinculoId=${alert.vinculoId}` : "/app/pendencias",
    });
  }
  const byPerson = new Map<string, typeof activeLinks>();
  for (const link of activeLinks) byPerson.set(link.pessoaId, [...(byPerson.get(link.pessoaId) ?? []), link]);
  for (const links of byPerson.values()) if (links.length > 1) {
    const link = links[0]!;
    result.push({
      id: `vinculo:${link.pessoaId}:multiplos`, codigo: "DUPLICIDADE_POSSIVEL", severidade: "ATENCAO",
      modulo: "VINCULO", pessoa: link.pessoa.nomeCompleto, pessoaId: link.pessoaId,
      unidadeId: link.unidadeId, ...(link.equipeId ? { equipeId: link.equipeId } : {}), status: "ABERTA",
      origem: "Cadastro de vínculos", descricao: `${links.length} vínculos ativos para a mesma pessoa.`,
...
export function registerPendings(app: FastifyInstance, db: PrismaClient) {
  app.get("/api/pendencias", async (req) => {
    ...
    items = items.filter((item) =>
      ...
      (!q.origem || item.origem.toLowerCase().includes(q.origem.toLowerCase())) &&
      (!q.pessoaId || item.pessoaId === q.pessoaId) &&
      (!q.unidadeId || item.unidadeId === q.unidadeId) &&
      (!q.equipeId || item.equipeId === q.equipeId) &&
      (!q.q || `${item.pessoa ?? ""} ${item.descricao} ${item.codigo}`.toLowerCase().includes(q.q.toLowerCase())),
    );
    const total = items.length;
    return { items: items.slice((q.page - 1) * q.pageSize, q.page * q.pageSize), total, page: q.page, pageSize: q.pageSize };
  });
  app.get("/api/pendencias/resumo", async () => {
    ...
    return {
      ...
      dependencias: items.filter((i) => i.codigo === "DEPENDENCIA_NAO_PUBLICADA").length,
    };
  });
}
