...
export const pessoaSchema = z
  .object({
    nomeCompleto: shortText,
    nomeSocial: optionalText,
    cpf: z
      .string()
...
export const vinculoSchema = z
  ...
  .strict()
  .superRefine((v, ctx) => {
    if ((v.status === "DESLIGADO") !== Boolean(v.dataDesligamento))
      ctx.addIssue({
        code: "custom",
        path: ["dataDesligamento"],
        message: "Desligamento exige status DESLIGADO e data.",
      });
    if (v.dataDesligamento && v.dataDesligamento < v.dataAdmissao)
      ctx.addIssue({
        code: "custom",
        path: ["dataDesligamento"],
